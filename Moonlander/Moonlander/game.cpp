...
bool Game :: justLanded() const
{
   ...
   if (fabs(xDiff) < margin)
   {
      ...
      if (yDiff < 4 && yDiff >= 0)
      {
		 ...
		 //Added this line for crashing on landing pad
...
void Game::advance()
{
	if (lander.isAlive() && !lander.isLanded())
	{
		...
		/*//Screen wrapping tried doing screen wrapping but
		    was unable to get it to work.
		if (lander.getPoint().getX() <= topLeft.getX())

				{
			lander.setPoint(Point(topLeft.getX(), lander.getPoint().getY()));
		}

		if (lander.getPoint().getX() >= bottomRight.getX())

		{
			lander.setPoint(Point(-bottomRight.getX(), lander.getPoint().getY()));
		}*/
...
void Game :: draw(const Interface & ui)
{
   ...
   if (lander.isLanded())
   {
	   //Changed point location for "You have successfully Landed"
      drawText(Point(0,30), "You have successfully landed!");
	  // Added congratulations text
	  drawText(Point(0, 75), "Congratulations!");
   }
